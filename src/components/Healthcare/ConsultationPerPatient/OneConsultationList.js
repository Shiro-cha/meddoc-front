...
export default function OneConsultationsListByPatient() {

  const contentRef = useRef(null);
  const location = useLocation();

  const [data, setData] = useState(location.state.data)
  const descriptionObject = JSON.parse(data.consultations);
  ...
  const generatePDF = () => {
    html2canvas(contentRef.current).then((canvas) => {
      ...
      pdf.save(`${data.id+"-"+data.patient.name+"-"+data.patient.firstname+'-'+ data.startDate}.pdf`);
    });
  };
    ...
    <>
      <div className="flex items-center justify-center h-full sm:m-68 border flex-col text-slate-200">
        <div className="flex items-center justify-center ">
          <h1 class="text-2xl font-semibold text-gray-800 mb-4 mr-2">Registre du visite medical du patient :<br></br></h1>

          <button onClick={generatePDF}>
            <img src={pdf} alt="pdf" className="w-8 h-8"></img>
          </button>
        </div>
        <div className="p-10 m-10 border text-black " ref={contentRef} >
            ...
            <div >
              ...
              <div className="flex space-x-4">
                ...
                <div className="border-b-2 border-gray-400 bottom-0 left-0 right-0 p">
                  {data.patient.name} {data.patient.firstname}
                </div>
              </div>
            </div>
              ...
              <div className="grid-cols-1 space-y-4">
                {consultation?.prescriptions.map((prescription, index) => (
                  <div key={index} className="max-w-32 border-b-2 border-gray-400 bottom-0 left-0 right-0 ">
                    {prescription.medicament} / {prescription.day}-{prescription.evening}-{prescription.night} (pendant {prescription.duration} jour(s))  / {prescription.schedule}
                  </div>
                ))}
